...
// ── Statistiques d'usage (visites, appareils uniques, installations) ──────────
// Stockées dans un seul document Firestore : m02_stats/compteurs
const STATS_DOC = doc(db, "m02_stats", "compteurs");

async function bumpStat(field, amount = 1) {
  try {
    await setDoc(STATS_DOC, { [field]: increment(amount) }, { merge: true });
  } catch (e) {
...
// Appelé une fois au chargement de l'app : comptabilise l'ouverture,
// détecte si l'app est lancée en mode "installée" (icône sur l'écran d'accueil),
// et compte les appareils uniques (1 seule fois par navigateur, via localStorage).
function trackAppUsage() {
  bumpStat("ouvertures_totales");

  const isStandalone =
    window.matchMedia("(display-mode: standalone)").matches ||
    window.navigator.standalone === true; // iOS Safari
  if (isStandalone) {
    bumpStat("lancements_depuis_icone");
  }

  if (!localStorage.getItem("m02_appareil_compte")) {
    localStorage.setItem("m02_appareil_compte", "1");
    bumpStat("appareils_uniques");
  }

  // Événement natif déclenché par Chrome/Android quand l'app est réellement
  // ajoutée à l'écran d'accueil (non disponible sur iOS/Safari).
...
function StatsModal({ onClose }) {
  ...
  const rows = [
    { key: "ouvertures_totales", label: "📈 Ouvertures totales de l'app", desc: "Cumul de toutes les fois où l'app a été chargée (tous appareils confondus)" },
    { key: "appareils_uniques", label: "👥 Appareils différents (approx.)", desc: "Nombre de navigateurs/téléphones distincts ayant déjà ouvert l'app au moins une fois" },
    { key: "lancements_depuis_icone", label: "🏠 Lancements depuis l'écran d'accueil", desc: "Ouvertures faites via l'icône installée (bon indicateur du nombre de personnes qui ont vraiment installé l'app)" },
    { key: "installations_confirmees_android", label: "✅ Installations confirmées (Android)", desc: "Comptées uniquement quand Chrome/Android détecte un ajout réel à l'écran d'accueil — non disponible sur iPhone" },
  ];

  return (
    <Modal onClose={onClose}>
      <h3 style={{ color: "#1a2e52", marginBottom: 4 }}>📊 Statistiques d'usage</h3>
      <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 16 }}>
        Ces chiffres sont des estimations (basées sur le navigateur), pas un suivi individuel des personnes.
      </p>
      {loading && <div style={{ color: "#6b7280", fontStyle: "italic" }}>Chargement…</div>}
      {!loading && rows.map(r => (
        <div key={r.key} style={{ marginBottom: 16, paddingBottom: 12, borderBottom: "1px solid #f0f4f8" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 14, fontWeight: 600, color: "#1a2e52" }}>{r.label}</span>
            <span style={{ fontSize: 22, fontWeight: 800, color: "#5bbfea" }}>{stats?.[r.key] || 0}</span>
          </div>
          <div style={{ fontSize: 11, color: "#6b7280", marginTop: 2 }}>{r.desc}</div>
        </div>
      ))}
      <p style={{ fontSize: 11, color: "#6b7280", marginTop: 4 }}>
        ℹ️ Sur iPhone, l'ajout à l'écran d'accueil ne peut pas être détecté directement par l'app — seuls les "lancements depuis l'écran d'accueil" donnent une bonne idée de l'adoption côté iOS.
      </p>
      <button onClick={onClose} style={{ width: "100%", padding: 12, marginTop: 4, background: C.lightGray, color: C.navy, borderRadius: 8, fontWeight: 600, border: "none" }}>
        Fermer
      </button>
    </Modal>
  );
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: C.lightGray }}>
      ...
      {isAdmin && (
        <div style={{ padding: "0 16px", marginTop: 8 }}>
          <button onClick={() => setShowStats(true)} style={{ width: "100%", padding: 10, background: C.white, color: C.navy, borderRadius: 10, fontWeight: 600, fontSize: 13, border: `1px solid ${C.skyBlue}55` }}>
